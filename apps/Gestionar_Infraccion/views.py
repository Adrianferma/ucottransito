...
def mapa_view(request):
    ...
    if request.method == 'POST':
        if (str(fechaFin)+'' != '') & (str(fechaInicio)+'' != ''):
            try:
                ...
                if str(Tipo)+'' == 'Infracciones':
                    ...
                    return render(request, 'mapa.html', {'infracciones': infracciones, 'contador' : contador,})
                else:
                    infracciones = Infraccion_Transito.objects.all().filter(Fecha_Infraccion__gte=fechaInicio, Fecha_Infraccion__lte=fechaFin)
                    contador = Infraccion_Transito.objects.all().filter(Fecha_Infraccion__gte=fechaInicio, Fecha_Infraccion__lte=fechaFin).count()
                    return render(request, 'mapa.html', {'infracciones': infracciones, 'contador' : contador,})
                ...
                return render(request, 'mapa.html', {'infracciones': infracciones})
        ...
        fechaHoy = date.today()
        infracciones = Infraccion_Transito.objects.all().filter(Fecha_Infraccion__gte=fechaHoy, Fecha_Infraccion__lte=fechaHoy)
...
def home(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'index.html', {'infracciones_hoy': infracciones})
